...
const ExploreItem: React.FC<ExploreItemProps> = ({
  ...
  return (
    <div
      className={clsx(
        `relative  ${item?.type === "creator"  ? "h-80 w-full" :item?.type === "coin"  ? "h-40 w-40" : "h-80 w-full"} bg-cover bg-center rounded-lg overflow-hidden group transform transition-transform duration-300 hover:scale-105`,
        containerClassName
      )}
      ...
      <div className="absolute inset-0 bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end items-center">
        <div className="text-center text-white px-4 mb-12">
          ...
          <p className="text-sm font-semibold">
            {item?.type === "creator" ? `${item?.followers} followers` : item?.type === "coin" ? `${item?.marketcap} marketCap` :`${item?.followers} followers` } 
          </p>
        </div>
